...
public class MainActivity extends Activity implements SensorEventListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mPressureSensorExists = packageManager
				.hasSystemFeature(PackageManager.FEATURE_SENSOR_BAROMETER);
		mCompassSensorExists = packageManager
	...
	public void updateData(View view) {
		...
		// write to log cat
		String dataString = mDate + "\t" + mTime + "\t" + mLatitude + "\t"
				+ mLongitude + "\t" + Float.toString(mPressureMillibars) + "\t"
			...
			if (inputStream != null) {
				...
				while ((receiveString = br.readLine()) != null) {
					...
					if (dataArray.length >= 6) {
						outputString = "Date: " + dataArray[0].toString()
								+ "  Time: " + dataArray[1] + "\n" + "Lat: "
								+ dataArray[2].toString() + " Long: "
								+ dataArray[3] + "\n" + "Pressure: "
								+ dataArray[4].toString() + " Az: "
								+ dataArray[5];
	...
	private void updateTextViews() {
		...
		if (mPressureSensorExists) {
			mTextPressure.setText("Pressure: "
					+ Float.toString(mPressureMillibars) + " mb");
		} else {
